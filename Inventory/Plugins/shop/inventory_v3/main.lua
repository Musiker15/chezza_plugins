        ...
        if item.type == 'item_weapon' then
            if xPlayer.getAccount(item.method).money >= item.count then 
                if not xPlayer.hasWeapon(item.name) then
                    ...
                    xPlayer.addWeapon(item.name, item.ammo or 1)
                    TriggerClientEvent('inventory:notify', src, 'success', ('%s <b>%s</b> gekauft!'):format(count, item.label))
                    DiscordLog(xPlayer.getName() .. " bought " .. item.label)
                else 
                    TriggerClientEvent('inventory:notify', src, 'error', 'You already have this weapon!')
                end
            else 
                TriggerClientEvent('inventory:notify', src, 'error', 'You don\t have enough money!')
            end
            ...
            if Config.PlayerWeight then
                if xPlayer.canCarryItem(item.name, count) then
                    if xPlayer.getAccount(item.method).money >= (count * item.count) then
                        ...
                        TriggerClientEvent('inventory:notify', src, 'success', ('%s <b>%s</b> gekauft!'):format(count, item.label))
                        DiscordLog(xPlayer.getName() .. " bought " .. count .. " " .. item.label) 
                    else 
                        TriggerClientEvent('inventory:notify', src, 'error', 'You don\t have enough money!')
                    end
                else 
                    TriggerClientEvent('inventory:notify', src, 'error', 'Inventar full!')
                end
            else 
        ...
        if item.type == 'item_account' then
            if item.name == 'cash' then
                if target.getMoney() >= count then
                    ...
                    TriggerClientEvent('inventory:notify', src, 'success', ('%s <b>%s</b> verschoben in <b>%s</b>!'):format(count, item.label, 'Dein Inventar'))
        ...
        elseif item.type == 'item_weapon' then
            if target.hasWeapon(item.name) then
                if not xPlayer.hasWeapon(item.name) then
                    ...
                    TriggerClientEvent('inventory:notify', src, 'success', ('<b>%s</b> verschoben in <b>%s</b>!'):format(item.label, 'Dein Inventar'))
                    DiscordLog(target.getName() .. " has transferred a " .. item.label .. " to " .. xPlayer.getName())
                else 
                    TriggerClientEvent('inventory:notify', src, 'error', 'Du hast diese Waffe bereits!')
                end
            end
        elseif item.type == 'item_standard' then
            if target.getInventoryItem(item.name).count >= count then
                if Config.PlayerWeight then
                    if xPlayer.canCarryItem(item.name, count) then
                        ...
                        TriggerClientEvent('inventory:notify', src, 'error', 'Inventar voll!')
                    end
                else 
                    local newCount = xPlayer.getInventoryItem(item.name).count + count
                    if newCount <= xPlayer.getInventoryItem(item.name).limit then
                        ...
                        TriggerClientEvent('inventory:notify', src, 'error', 'Limit erreicht')
                    end
                end
            end
        end
        ...
        if item.type == 'item_account' then
            if item.name == 'cash' then
                if xPlayer.getMoney() >= count then
                    ...
                    TriggerClientEvent('inventory:notify', src, 'success', ('%s <b>%s</b> verschoben in <b>%s</b>!'):format(count, item.label, otherInv.title))
        ...
        elseif item.type == 'item_weapon' then
            if xPlayer.hasWeapon(item.name) then
                if not target.hasWeapon(item.name) then
                    ...
                    TriggerClientEvent('inventory:notify', src, 'success', ('<b>%s</b> verschoben in <b>%s</b>!'):format(item.label, otherInv.title)) 
                    DiscordLog(xPlayer.getName() .. " has added a " .. item.label .. " to " .. target.getName())
                else 
                    TriggerClientEvent('inventory:notify', src, 'error', 'Der Spieler hat diese Waffe bereits')
                end
            end
        elseif item.type == 'item_standard' then
            if xPlayer.getInventoryItem(item.name).count >= count then
                if Config.PlayerWeight then
                    if target.canCarryItem(item.name, count) then
                        ...
                        TriggerClientEvent('inventory:notify', src, 'error', 'Das Inventar des Spielers ist voll!')
                    end
                else 
                    ...
                    if newCount <= target.getInventoryItem(item.name).limit then
                        ...
                        TriggerClientEvent('inventory:notify', src, 'error', 'Das Inventar Limit des Spielers ist erreicht!')
                    end
                end
            end
        end
...
RegisterNetEvent('inventory:removeItem', function(item, count, coords)
    ...
    if item.type == 'item_account' then
        if item.name == 'cash' then
            if xPlayer.getMoney() >= count then
                ...
                TriggerClientEvent('inventory:notify', src, 'success', ('%s <b>%s</b> <b>%s</b>!'):format(count, item.label, 'weg geworfen'))
    ...
    elseif item.type == 'item_weapon' then
        if xPlayer.hasWeapon(item.name) then
            ...
            TriggerClientEvent('inventory:notify', src, 'success', ('<b>%s</b> <b>%s</b>!'):format(item.label, 'weg geworfen'))
...
ESX.RegisterServerCallback('inventory:getName', function(source, cb, playerId)
    if playerId then 
